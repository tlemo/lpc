...
(*$c+,t-,d-,l-*)
  ...
  * Under Unix and DOS/Windows, the command     *
  * line is:                                    *
  *                                             *
...
program pascalcompiler(output,source,prr);
...
const displimit = 20; maxlevel = 10;
   ...
   sethigh     =     47;
   setlow      =      0;
   ordmaxchar  =     255{63}; { standard 8 bit ASCII limits }
   ordminchar  =      0;
   {maxint      =  32767;} { let this be the native value [sam] }
   lcaftermarkstack = 5;
   fileal      = charal;
...
var
    source, prr: text; (* comment this out when compiling with pcom *)
...
(*-------------------------------------------------------------------------*)
  ...
  begin
    ...
    if list and (not eof(source)) then
  ...
  procedure insymbol;
    ...
    begin if eol then
      ...
      if not eof(source) then
       begin eol := eoln(source); read(source,ch);
       ...
       begin nextch;
         if ch = '*' then
           begin nextch;
             if ch = '$' then options;
             repeat
               while (ch <> '*') and not eof(source) do nextch;
               nextch
             until (ch = ')') or eof(source);
             nextch; goto 1
           end;
         sy := lparent; op := noop
       end;
...
1:  fcp := lcp
  ...
  procedure block(fsys: setofsys; fsy: symbol; fprocp: ctp);
    ...
    begin
      if not eof(source) then
        begin while not(sy in fsys) and (not eof(source)) do insymbol;
    ...
    begin (*procdeclaration*)
      ...
      if sy = forwardsy then
        ...
        begin lcp^.forwdecl := false; mark(markp);
          ...
          until (sy in [beginsy,procsy,funcsy]) or eof(source);
  ...
  begin (*block*)
    ...
    until (sy in statbegsys) or eof(source);
    dp := false;
    if sy = beginsy then insymbol else error(17);
    repeat body(fsys + [casesy]);
      if sy <> fsy then
        begin error(6); skip(fsys) end
    until ((sy = fsy) or (sy in blockbegsys)) or eof(source);
  end (*block*) ;
